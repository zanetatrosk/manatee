...
export const handlers = [
  rest.get("/races", (req, res, ctx) => {
    ...
    let filteredRaces = races.filter((race: Race) => sources?.includes(race.source.id));
  ...
  rest.get("/backgrounds", (req, res, ctx) => {
    const sources = req.url.searchParams.get("source");
    let filteredBackgrounds = sources ? backgrounds.filter((background: Background) => sources?.includes(background.source.id)) : backgrounds;
  ...
  rest.get("/classes", (req, res, ctx) => {
    const sources = req.url.searchParams.get("source");
    let filteredClasses = sources ? classes.filter((c) => sources?.includes(c.source.id)) : classes;
    return res(ctx.status(200), ctx.json(filteredClasses));
  }),

  rest.get("/languages", (req, res, ctx) => {
    const sources = req.url.searchParams.get("source");
    let filteredLanguages = sources ? languages.filter((l) => sources?.includes(l.source.id)) : languages;
    return res(ctx.status(200), ctx.json(languages));
  }),

  rest.get("/tools", (req, res, ctx) => {
    const sources = req.url.searchParams.get("source");
    let filteredTools = sources ? proficiencyTools.filter((t) => sources?.includes(t.source.id)) : proficiencyTools;
